...
async function checkForUpdate() {
  ...
  try {
    ...
    if (ui.title) ui.title.textContent = `Open Cloud ${availableUpdate.version}`;
    if (ui.subtitle) ui.subtitle.textContent = `Current version: ${availableUpdate.currentVersion}`;
    if (ui.message) ui.message.textContent = availableUpdate.body || 'A signed update is ready to install.';
    if (ui.install) ui.install.style.display = 'block';
  } catch (error) {
...
export function initUpdater(button, accountDropdown) {
  ...
  ui.modal?.querySelector('.modal-overlay')?.addEventListener('click', () => ui.modal?.classList.add('hidden'));
