...
1. **Download the DMG**
   - Click the **macOS (.dmg)** button above, or go to the [Releases page](https://github.com/sebastianmiletic/opencloud/releases/latest) and download `OpenCloud_3.9.7_universal.dmg`
...
1. **Download the Installer**
   - Download `OpenCloud_3.9.7_x64-setup.exe` for Intel/AMD PCs or `OpenCloud_3.9.7_arm64-setup.exe` for Windows on ARM.
...
1. Download `OpenCloud_3.9.7_amd64.AppImage` for Intel/AMD64 Linux or `OpenCloud_3.9.7_aarch64.AppImage` for ARM64 Linux. Matching `.deb` packages are also available for Debian/Ubuntu.
2. For the AppImage, make it executable and launch it:

   ```bash
   chmod +x OpenCloud_3.9.7_amd64.AppImage
   ./OpenCloud_3.9.7_amd64.AppImage
   ```

   The app uses the desktop's native display scale; no force-scale launch flag is required.

3. On Debian/Ubuntu, install the matching package with `sudo apt install ./OpenCloud_3.9.7_amd64.deb` or `sudo apt install ./OpenCloud_3.9.7_arm64.deb`.
...
`OPEN_CLOUD_WEBKIT_RENDERER=accelerated ./OpenCloud_3.9.7_aarch64.AppImage`. For a last-resort CPU
